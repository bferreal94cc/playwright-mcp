...
The product ships on Google Agents ADK first but must not be welded to it --
enterprise buyers in both launch verticals routinely mandate a specific cloud.
Everything above this module talks to :class:`LLMProvider`, so swapping Gemini
for Claude (or for a stub in CI) is a constructor argument.

Cloud SDKs are imported lazily inside their providers, so the core package
installs and tests with no third-party dependencies at all.
"""
...
class StubProvider(LLMProvider):
    ...
    Returns canned responses keyed by substring, falling back to a fixed
    string. Keeps tests hermetic and lets the whole system run with no API key.
    """
...
class GoogleADKProvider(LLMProvider):
    """Gemini via the Google Agents ADK / GenAI SDK.

    Requires the ``google`` extra and ``GOOGLE_API_KEY`` (or ADC in a GCP
    runtime).
    """
    ...
    def _ensure_client(self):
        ...
        try:
            ...
            raise RuntimeError(
                "GoogleADKProvider needs the 'google' extra: pip install 'hr-scheduling-agent[google]'"
            ) from exc
    ...
    def _ensure_client(self):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "pip install 'hr-scheduling-agent[anthropic]'"
            ) from exc
...
def provider_from_env(default: LLMProvider | None = None) -> LLMProvider:
    ...
    ``HR_AGENT_LLM`` selects explicitly (``google``/``anthropic``/``stub``).
    Otherwise the first cloud whose API key is present wins, and failing that
    we fall back to the stub so the system always starts.
    """
    choice = os.environ.get("HR_AGENT_LLM", "").strip().lower()
    if choice == "google":
