...
Every scheduling decision, approval and override lands here with the reasoning
that produced it. Healthcare and multi-state retail customers need to show a
regulator *why* a given person was put on a given shift, months later.

Entries are chained by hash: each entry commits to the one before it, so
editing or deleting history invalidates every entry after the change and
:meth:`AuditLog.verify` reports exactly where the chain broke. This detects
tampering; it does not prevent it. Durable storage and access control are the
    ...
    def verify(self) -> VerificationResult:
        ...
        for index, entry in enumerate(self.entries):
            ...
            if entry.previous_hash != expected_previous:
                return VerificationResult(False, index, f"entry {index} does not chain to {index - 1}")
