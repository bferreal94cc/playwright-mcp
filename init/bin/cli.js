...
// Generated by metaharness — the `init` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx init`
...
// "bin" + "files" fields in package.json), so `npx init` works the moment
...
const HARNESS_NAME = 'init';

/** `init init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `init doctor` — verify the install end-to-end (kernel + host resolve). */
